...
Customer requests persist as the same identity in `requests` and enterprise `acorn_state` (PROJECT). Evidence goes through the existing evidence registry. Connector execution remains BLOCKED without human authorization. `POST /api/v1/runtime/external` ignores client `human_authorized` / `authority` / `base_url`. WRITE/MONEY/PUBLISH/SIGN/DELETE/MERGE stay locked server-side. HTTP availability is not LIVE proof.
