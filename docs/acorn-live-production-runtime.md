...
Customer requests persist as the same identity in `requests` and enterprise `acorn_state` (PROJECT). Evidence goes through the existing evidence registry. Named capabilities are not marked existing unless code or a catalog match is observed. Detected gaps persist as CAPABILITY records with `exists:false` and state `ABSENT`. Connector execution remains BLOCKED without human authorization. `POST /api/v1/runtime/external` ignores client `human_authorized` / `authority` / `base_url`. WRITE/MONEY/PUBLISH/SIGN/DELETE/MERGE stay locked server-side. Connector idempotency (`acorn_idempotency`) replays the public result; it does not mint authority. Stale RUNNING jobs are requeued. HTTP availability is not LIVE proof.
