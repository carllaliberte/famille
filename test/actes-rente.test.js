...
describe("rente — acte classique si champ juge manque", () => {
  it("names the lock: missing field stays classique, not a four-card sale", () => {
    ...
    assert.match(actes, /Pas « live »/);
    assert.match(actes, /Pas PRÉSENT/);
    assert.match(actes, /404 ≠ carte juge/);
    assert.match(actes, /404 ≠ trou epsilon \/ horizon/);
    assert.doesNotMatch(actes, /404 → HOLD Carl/);
    assert.doesNotMatch(actes, /404 = HOLD Carl/);
    assert.match(rente, /Les certitudes ont une date de fin/);
    ...
    assert.match(rente, /sur la vitrine = 404 HTML/);
    assert.match(rente, /Pas un produit/);
    assert.match(rente, /Pas « live »/);
    assert.match(rente, /Pas PRÉSENT/);
    assert.match(rente, /404 ≠ carte juge/);
    assert.match(rente, /404 ≠ trou epsilon \/ horizon/);
