...
describe("codex autonomous worker", () => {
  ...
  it("writes a full-repo sandbox and runs Astra Codex with danger-full-access", () => {
    ...
    assert.match(prompt, /repository is in scope/i);
    ...
    assert.match(prompt, /LIVE_VERIFIED/);
    ...
    assert.equal(writeCall.args.includes("model=cohere/north-mini-code:free"), true);
    assert.match(cfg, /model = "cohere\/north-mini-code:free"/);
    assert.doesNotMatch(cfg, /nemotron-3\.5-lightning/);
    assert.doesNotMatch(cfg, /openrouter\/free/);
    assert.doesNotMatch(cfg, /gpt-oss-20b/);
    assert.equal(resolveOpenRouterModel({ env: { CODEX_MODEL: "google/gemini-2.5-flash" } }), "google/gemini-2.5-flash");
    assert.equal(resolveOpenRouterModel({ env: { CODEX_MODEL: "openrouter/free" } }), "cohere/north-mini-code:free");
    assert.equal(resolveOpenRouterModel({ env: { CODEX_MODEL: "openai/gpt-oss-20b:free" } }), "cohere/north-mini-code:free");
