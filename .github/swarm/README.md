...
`claim.mjs` · `cadence.mjs` · `workforce.mjs` — claim expire ; 1 PR / repo (`cursorGate` READY ou RAS) ; IDLE avant recruter. IDLE ≠ extinction. `activate()` skip/quota = CAPABILITY UNAVAILABLE, pas PROJECT BLOCKED. Jamais merge.
