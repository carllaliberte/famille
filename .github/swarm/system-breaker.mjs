 ...
 *
 * Control plane for the boundary between external ingress (including the
 * future quantum connector) and the protected Acorn execution/source-of-record
 * domain.
 *
 * Modes:
 ...
 * variable ACORN_SYSTEM_MODE. It is deliberately outside the source tree so
 * changing OFF does not itself require a source-of-record write.
 */
...
export function resolveMode(env = process.env) {
  ...
  // Invalid control state is fail-closed.
  return MODES.OFF;
}
...
export function controlState(env = process.env) {
  ...
  return {
    ...
    normal: mode === MODES.RUN,
    quantum_ingress_allowed: mode !== MODES.OFF,
...
export function assertSystemMayProceed({ env = process.env, origin = "unknown", action = "execute" } = {}) {
  ...
  if (state.mode === MODES.OFF) {
    const error = new Error(
      `GLOBAL_BREAKER_OFF: ${action} blocked for origin=${origin}. ` +
      "No execution, dispatch, write, merge, or downstream propagation is permitted."
    );
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (command) console.log(JSON.stringify(commandResult(command), null, 2));
  else console.log(JSON.stringify(controlState(), null, 2));
}
