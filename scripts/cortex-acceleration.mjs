...
export function runAccelerationFabric(input = {}) {
  ...
  if (sovereignty.breaker.status === "HOLD_HUMAN") {
    return {
      ...
      nvidia: { state: "CHANNEL_NOT_PRESENT", probe: { live: false }, attempt: sovereignty.nvidia },
      unknown: { identity: "FUTURE_ACCELERATOR_X", kind: "UNKNOWN" },
      compared: compareResources({ a: { id: "cpu-local" }, b: { id: "nvidia-gpu" }, measurements: { measured: false } }),
      lockin: ["provider", "model", "hardware", "protocol", "adapter", "intelligence"].map((kind) => noLockIn({ kind })),
      sovereignty,
      secrets_used: false,
      nvidia_called: false,
      fake_success: false,
      ...
      paid_forbidden: true,
      invoke: { reason: "CHANNEL_NOT_PRESENT", live: false },
      failover: { status: "HOLD_HUMAN", fake_success: false, live: false },
      zero_cost: true,
      live: false,
      auto_merge: false,
      authority: "carl",
