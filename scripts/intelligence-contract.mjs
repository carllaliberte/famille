...
export function describeIntelligence(input = {}) {
  ...
  return {
    ...
    interfaces: list(input.interfaces),
    context_window: input.context_window ?? null,
    reasoning_characteristics: input.reasoning_characteristics ?? null,
    latency_characteristics: input.latency_characteristics ?? null,
    throughput: input.throughput ?? null,
    availability: input.availability || "UNKNOWN",
    cost_class: input.cost_class || (input.cost ?? (lane === "paid" ? "PAID" : lane === "free" ? "PROVIDER_FREE" : "LOCAL_FREE")),
    resource_requirements: input.resource_requirements ?? null,
    hardware_requirements: input.hardware_requirements ?? null,
    software_requirements: input.software_requirements ?? null,
    security_properties: input.security_properties || [],
    verification_capabilities: list(input.verification_capabilities),
    provenance: input.provenance || { source: "describeIntelligence", invented: false },
    version: input.version || "UNKNOWN",
    temporal_validity: input.temporal_validity || { discovered_at: null, verified_at: null, last_confirmed: null, expires_at: null },
    observed_performance: input.observed_performance ?? null,
