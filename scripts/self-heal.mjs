 ...
 *
 * This layer may automatically retry/restart autonomous execution and hand a
 * measured failure back to the Codex repair worker. It never merges, never
 * changes protected governance, never exposes secrets, and never declares LIVE.
 * Human intervention is reserved for decisions, secrets, authorization,
 * payment, or a repair that cannot be safely automated.
 */
...
export function recoveryPlan({ failureClass, attempt = 0, maxAttempts = 3, breaker = "RUN" } = {}) {
  ...
  if (mode === "OFF") {
    return { version: SELF_HEAL_VERSION, action: "STOP", reason: "GLOBAL_BREAKER_OFF", retry: false, human_required: false };
  }
  ...
  if (attempt >= maxAttempts) {
    return { version: SELF_HEAL_VERSION, action: "ESCALATE", reason: "REPAIR_ATTEMPTS_EXHAUSTED", retry: false, human_required: false };
  }
  switch (failureClass) {
    case "TRANSIENT":
      return { version: SELF_HEAL_VERSION, action: "RETRY", reason: "TRANSIENT_FAILURE", retry: true, human_required: false };
    case "ENVIRONMENT":
      return { version: SELF_HEAL_VERSION, action: "RESTART_WORKER", reason: "ENVIRONMENT_FAILURE", retry: true, human_required: false };
    case "CODE_OR_TEST":
      return { version: SELF_HEAL_VERSION, action: "DISPATCH_REPAIR", reason: "MEASURED_CODE_OR_TEST_FAILURE", retry: true, human_required: false };
    default:
      return { version: SELF_HEAL_VERSION, action: "DIAGNOSE", reason: "UNCLASSIFIED_FAILURE", retry: true, human_required: false };
  }
}
