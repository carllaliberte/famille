...
export function taskGraph(tasks=[]){
  ...
  const terminal = new Set(["SUCCEEDED", "FAILED", "CANCELLED", "RUNNING", "BLOCKED"]);
  return tasks.map((t) => {
    if (terminal.has(t.state)) return t;
...
export function guardExecutionEffect(effect){
  ...
  if (name.startsWith("AUTO_") && EXECUTION_POLICIES[name] === false) {
    throw new Error(`FORBIDDEN_AUTOMATIC_EFFECT:${name}`);
  }
