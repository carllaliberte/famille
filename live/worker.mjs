...
import { ingestPersistedRequests, jobRetryState, reclaimStaleJobs } from "./worker-jobs.mjs";
...
async function claim() {
  ...
  try {
    await c.query("BEGIN");
    const q = await c.query("SELECT id FROM acorn_jobs WHERE state='QUEUED' AND attempts<max_attempts ORDER BY created_at FOR UPDATE SKIP LOCKED LIMIT 5");
    const jobs = [];
    for (const row of q.rows) {
      const r = await c.query("UPDATE acorn_jobs SET state='RUNNING',worker_id=$1,attempts=attempts+1,updated_at=$2 WHERE id=$3 RETURNING *", [WORKER_ID, now(), row.id]);
      jobs.push(r.rows[0]);
    }
...
async function execute(job) {
  ...
  if (!r.rows[0]) {
    await pool.query("UPDATE acorn_jobs SET state='FAILED',error=$1,updated_at=$2,finished_at=$2 WHERE id=$3", ["REQUEST_NOT_FOUND", now(), job.id]);
    return;
  }
  try {
    const body = typeof r.rows[0].body === "string" ? JSON.parse(r.rows[0].body) : r.rows[0].body;
    ...
    const evidence = registerEvidence({
      ...
      claim: "worker_execution",
      source: "acorn-live-worker",
      kind: "OBSERVATION",
      strength: 1,
      margin: 0.1,
...
async function shutdown() {
  ...
  stopping = true;
  try {
    await pool.query("UPDATE acorn_jobs SET state='QUEUED',worker_id=NULL,updated_at=$1 WHERE state='RUNNING' AND worker_id=$2", [now(), WORKER_ID]);
  } finally {
    await db.close();
    process.exit(0);
  }
...
while (!stopping) {
  try {
    await reclaimStaleJobs((sql, params) => pool.query(sql, params));
    await ingest();
    for (const j of await claim()) if (!stopping) await execute(j);
  } catch {
    console.error("WORKER_TICK_ERROR");
  }
